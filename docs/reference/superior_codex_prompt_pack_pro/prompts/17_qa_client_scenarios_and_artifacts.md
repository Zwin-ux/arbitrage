# Prompt 17: Expand QA client scenarios so product quality is observable, not guessed
...
Strengthen the QA client as a product-evidence system that proves Superior works as a guided machine, not just a collection of passing unit tests.
...
1. Audit the existing QA scenarios and identify missing high-value product flows.
2. Add scenarios for any major new surfaces introduced by recent prompts, especially Bot Bay, Replay Lab, or decision traces if they exist.
3. Improve exported artifact structure and text rendering so a human reviewer can understand what happened.
4. Document how QA artifacts should be reviewed before release.

## Constraints

- Keep QA deterministic and local.
- Do not hide failures behind vague summaries.
- Prefer reviewable artifacts over opaque pass/fail behavior.
...
- QA gives a stronger product-level readout.
- Release confidence improves.
- New features are easier to validate.
