# Prompt 13: Introduce a clean bot recipe format and local forking workflow
...
Create a simple, explicit recipe format for bots that can support local duplication, modification, and future sharing without turning the app into a scripting free-for-all.

## Files to inspect first

- `src/market_data_recorder_desktop/score_attack.py`
- `src/market_data_recorder_desktop/app_types.py`
- `src/market_data_recorder_desktop/profiles.py`
...
1. Design a recipe/config format for bot presets that is explicit, serializable, and future-proof enough for local fork/duplicate behavior.
2. Keep the recipe language grounded in current concepts like strategy family, min net edge, target stake, max assignments, and route preference.
3. Add a first local duplication/fork flow if feasible, or at least the data model and service seams for it.
4. Update docs so the recipe model is understandable to contributors and future UI work.

## Constraints

- Do not invent a giant DSL.
- Use today’s bot system as the base, not a speculative rewrite.
...
- Bots can evolve from hardcoded blueprints into explicit recipes.
- Future Bot Garage work becomes much easier.
- The model remains simple and trustworthy.
