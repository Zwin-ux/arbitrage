# Prompt 11: Build the foundation of Replay Lab using existing recorder and paper-run data
...
Add a first Replay Lab foundation so Superior begins to feel like a machine you can study, not just operate once.

## Files to inspect first

- `src/market_data_recorder/replay.py`
- `src/market_data_recorder/service.py`
...
1. Design a minimal Replay Lab surface that uses existing data rather than inventing a huge new system.
2. Allow a user to inspect a previous paper session or recorded run as a replayable timeline or ordered event review.
3. Expose route staging, entry, bank, blocked, and session completion events in a clearer narrative flow.
4. Create the right abstractions for future expansion into richer historical replays.
5. Add tests for the new replay data shaping logic.

## Constraints

- Do not build a fake media player.
- Keep V1 replay simple and evidence-based.
- Avoid huge schema churn unless clearly justified.
...
- Superior begins to have a film-room quality.
- Past sessions are easier to learn from.
- The codebase now has a real seam for future replay expansion.
