# Prompt 04: Rebuild the shell, navigation hierarchy, and state overlays to feel premium
...
Upgrade the desktop shell so the header, profile bar, nav, content frame, and state overlays feel like a premium authored EXE instead of generic tabs.
...
1. Audit the current shell composition and tab order against the README and actual product loop.
2. Refine the shell hierarchy so the header, profile selector, top state, and content frame read like a cabinet console.
3. Improve the visual treatment and logic of shell-level states: empty, waiting, route ready, blocked, error, splash.
4. Make the nav labels, ordering, and emphasis support the real product loop: Hangar → Loadout/Bot Bay → Scanner → Paper Runs/Score → Live Gate → diagnostics/docs.
5. Add test coverage for the new shell state behavior and any ordering or visibility rules.

## Constraints

- No giant CSS/Qt stylesheet chaos; keep changes legible.
- Do not overload the shell with too many decorative elements.
- The shell must feel authored but calm.
...
- The app frame feels like a real product shell.
- Navigation supports the actual learning loop.
- State overlays are visually distinct and semantically clear.
