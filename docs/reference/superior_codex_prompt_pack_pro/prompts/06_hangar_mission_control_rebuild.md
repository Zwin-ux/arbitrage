# Prompt 06: Make the Hangar a true mission-control screen
...
Turn the Hangar into the unmistakable center of the product: a mission-control surface that tells the user what state the machine is in, what matters next, and why.

## Files to inspect first

- `src/market_data_recorder_desktop/ui/hangar.py`
- `src/market_data_recorder_desktop/window.py`
- `src/market_data_recorder_desktop/controller.py`
- `src/market_data_recorder_desktop/diagnostics.py`
...
1. Refine the Hangar information hierarchy around readiness, next step, first-pass checklist, golden path, and machine state.
2. Make the action bar and console feel like the only actions that matter on first launch.
3. Use signal badges and state cards in a cleaner, more disciplined way.
4. Ensure the first-run experience matches the product docs: equip Polymarket, record sample, scan route, start paper session, bank score.
5. Update tests and product copy to match the improved Hangar flow.

## Constraints

- Do not overfill the Hangar with secondary analytics.
- Keep the mission-control feeling strong.
- The screen should be instantly legible to a first-time user.
...
- The Hangar is obviously the product’s home base.
- The first-run loop is visible and motivating.
- The machine feels alive without becoming noisy.
