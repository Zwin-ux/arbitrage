# Prompt 22: Expand CI into a clearer Windows + site + future macOS matrix
...
Refine GitHub Actions and release validation so the repo has a cleaner long-term CI story that can grow to include macOS without diluting Windows confidence.
...
1. Audit the current CI jobs for readability, separation of concerns, and extension points.
2. Improve job naming and structure so future macOS work can slot in cleanly.
3. Add any safe validation improvements that make failures easier to diagnose.
4. If a macOS lane is added, keep it clearly labeled as PoC or non-release-ready until the repo truly supports more.

## Constraints

- Do not pretend the repo is production-ready on macOS unless it is.
- Preserve the current Windows release confidence.
- Keep CI maintainable.
...
- CI tells a clearer story.
- Windows, QA, and site checks remain strong.
- The repo has an obvious path to broader platform coverage.
