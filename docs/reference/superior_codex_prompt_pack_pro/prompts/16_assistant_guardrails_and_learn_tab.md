# Prompt 16: Upgrade Learn tab and assistant guardrails without compromising trust
...
Make the Learn surface and assistant behavior more useful and premium while preserving the hard separation between coaching and execution.

## Files to inspect first

- `src/market_data_recorder_desktop/bot_services.py`
- `src/market_data_recorder_desktop/window.py`
- `src/market_data_recorder_desktop/credentials.py`
- `tests/test_qa_client.py`
- `tests/test_desktop_ui.py`
- `docs/risk-model.md`
- `docs/privacy-and-secrets.md`

## What to change

1. Audit the assistant and Learn tab copy, state handling, and visible capabilities.
2. Make it easier for a user to understand what the coach can do and what it explicitly cannot do.
3. Improve the UI treatment so Learn feels like a premium guided surface, not an afterthought.
4. Add or strengthen tests around coach guardrails and no-trade boundaries.

## Constraints

- The coach must remain read-only and non-executing.
- Do not create any blurred line between education and live trading actions.
- Secrets must remain keychain-backed.
...
- The assistant feels more useful and more trustworthy.
- The Learn tab has a real role in the product.
- Guardrails are more visible, not less.
