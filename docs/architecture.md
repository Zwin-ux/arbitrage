...
- `market_data_recorder_desktop`: guided onboarding, profile store, keychain-backed credentials, scanner, paper bots, unlock flow, coach, diagnostics, Qt UI
...
- Scanner and paper-bot flows sit on top of local DuckDB recorder data.
...
- `src/market_data_recorder_desktop/ui/hangar.py` owns the Hangar control-surface composition: mission header, checklist, action bar, system console, progress panel, and telemetry group.
...
- `score_attack.py` owns the paper session machine:
  - starter bot blueprints and slot previews
  - starter registry visibility
  - unlock track evaluation
  - portfolio snapshots
  - tactical decision traces for paper sessions
- The intended user loop is:
  - equip modules
  - arm starter bots
  - refresh the scanner
  - stage a paper session
  - bank score
  - inspect unlock progress and tactical traces
